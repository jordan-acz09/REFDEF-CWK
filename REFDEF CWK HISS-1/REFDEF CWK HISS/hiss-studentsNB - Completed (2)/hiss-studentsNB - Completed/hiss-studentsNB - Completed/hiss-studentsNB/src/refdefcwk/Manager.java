    ...
    public String hireStaff(String name)
{
    ...
    } else if (team.containsKey(name) && staff.getState() == StaffState.WORKING) {
    ...
    public String doJob(int jbNo)
{
    ...
    if (staff != null) {
        if (staff.getExperience() > job.getExperienceRequired()) { // changed to '>'
            int pay = job.getHours() * staff.getHourlyRate();
            account += pay;
            staff.setState(StaffState.ONLEAVE);
    ...
    private void setupStaff()
{
    ...
    allStaff.put("Sam", new Apprentice("Sam", 1, 50,10,"Olenka" ));
}
